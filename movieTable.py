...
class MovietablePage(ctk.CTkFrame):
    ...
    def _build_nav(self):
        ...
        ctk.CTkButton(
            ...
            fg_color="transparent", text_color=TEXT_GRAY, corner_radius=16,
            font=("Trebuchet MS", 11, "bold"),
            command=lambda: self.app.show_page("genreanalyze")
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            header_frame, text="Movie Database",
            font=("Helvetica", 36, "bold"), text_color=TEXT_WHITE
        ).pack()
    ...
    def render_table(self):
        ...
        for movie in movies_to_show:
            card = ctk.CTkFrame(
                self.rows_frame, fg_color="#FFFFFF",
                corner_radius=10, height=90, cursor="hand2"
            )
            ...
            poster_lbl = ctk.CTkLabel(
                card, text="🎬", fg_color="#E0E0E0",
                width=50, height=75, corner_radius=5
            )
            ...
            rat_val = ctk.CTkLabel(
                rt_frame, text=f"{movie.get('rating', 'N/A')}",
                font=("Helvetica", 18, "bold"), text_color="#8A4B1A"
            )
            rat_val.pack()
            
            rat_text = ctk.CTkLabel(
                rt_frame, text="⭐ IMDb",
                font=("Trebuchet MS", 10), text_color="#AAAAAA"
            )
            rat_text.pack()
        ...
        ctk.CTkButton(
            self.pagination_frame, text="◀ Prev", width=100,
            fg_color=ACCENT, command=self.prev_page,
        ...
        ctk.CTkButton(
            self.pagination_frame, text="Next ▶", width=100,
            fg_color=ACCENT, command=self.next_page,
