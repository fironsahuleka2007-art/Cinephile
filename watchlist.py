...
class WatchlistPage(ctk.CTkFrame):
    ...
    # ── HELPER: Load & simpan user data dari users.json ──
    def _load_user_profile_data(self):
        try:
            if os.path.exists("users.json"):
                with open("users.json", "r") as f:
                    all_users = json.load(f)
                return all_users.get(self.current_user, {})
        except Exception:
            pass
        return {}

    def _save_user_profile_data(self, data: dict):
        try:
            all_users = {}
            if os.path.exists("users.json"):
                with open("users.json", "r") as f:
                    all_users = json.load(f)
            all_users[self.current_user] = data
            with open("users.json", "w") as f:
                json.dump(all_users, f, indent=4)
        except Exception as ex:
            print(f"[watchlist] gagal simpan users.json: {ex}")
            

    # ── HELPER: Load foto profile bulat ──────────────────────────────────────
    def _get_profile_image(self, size=34):
        user_data  = self._load_user_profile_data()
        candidates = []
        avatar_path = user_data.get("avatar_path", "")
        if avatar_path:
            candidates.append(avatar_path)
        for ext in ("png", "jpg", "jpeg"):
            candidates.append(f"profile_{self.current_user}.{ext}")
    ...
    def _build_nav(self):
        ...
        user_badge.pack(side="right", padx=20)
        self._nav_user_badge = user_badge

        self._render_nav_badge(user_badge)

    def _render_nav_badge(self, user_badge):
        for w in user_badge.winfo_children():
            w.destroy()

        nav_photo = self._get_profile_image(size=34)
        if nav_photo:
            photo_lbl = ctk.CTkLabel(user_badge, image=nav_photo, text="")
            photo_lbl.pack(side="left", padx=(6, 4), pady=6)
            photo_lbl.bind("<Button-1>", lambda e: self._open_profile_popup())
        else:
            ctk.CTkLabel(
                user_badge, text="👤",
        ...
        self.user_name_lbl = ctk.CTkLabel(
            user_badge, text=self.current_user,
            font=("Trebuchet MS", 12, "bold"), text_color=TEXT_WHITE
        )
        self.user_name_lbl.pack(side="left", padx=(0, 12), pady=8)

        user_badge.bind("<Button-1>", lambda e: self._open_profile_popup())
        self.user_name_lbl.bind("<Button-1>", lambda e: self._open_profile_popup())

    def _build_nav_badge(self):
        if hasattr(self, "_nav_user_badge") and self._nav_user_badge.winfo_exists():
            self._render_nav_badge(self._nav_user_badge)

    # ── PROFILE POPUP ─────────────────────────────────────────────────────────
    def _open_profile_popup(self):
        ...
        # ── Header ────────────────────────────────────────────────────────────
        header = ctk.CTkFrame(popup, fg_color="#252525", corner_radius=0)
        header.pack(fill="x")

        avatar_frame = ctk.CTkFrame(header, fg_color="transparent", cursor="hand2")
        avatar_frame.pack(pady=(16, 4))

        def _render_avatar():
            for w in avatar_frame.winfo_children():
                w.destroy()
            photo = self._get_profile_image(size=84)
            if photo:
                lbl = ctk.CTkLabel(avatar_frame, image=photo, text="")
                lbl.pack()
                lbl.bind("<Button-1>", lambda e: _upload_photo())
            else:
                outer = ctk.CTkFrame(avatar_frame, fg_color="#333333",
                                     width=84, height=84, corner_radius=42)
                outer.pack()
                outer.pack_propagate(False)
                ctk.CTkLabel(outer, text="👤", font=("Arial", 36),
                             text_color="#888").place(relx=0.5, rely=0.5, anchor="center")
            ctk.CTkLabel(avatar_frame, text="Change Photo",
                         font=("Trebuchet MS", 9), text_color="#666666").pack(pady=(2, 0))
            avatar_frame.bind("<Button-1>", lambda e: _upload_photo())
            for child in avatar_frame.winfo_children():
                try: child.bind("<Button-1>", lambda e: _upload_photo())
                except: pass

        def _upload_photo():
            file_path = filedialog.askopenfilename(
                parent=popup, title="Choose Profile Photo",
                filetypes=[("Image files", "*.png *.jpg *.jpeg *.webp *.bmp")]
            )
            if not file_path:
                return
            try:
                save_path = f"avatar_{self.current_user}.png"
                ...
                user_data["avatar_path"] = save_path
                self._save_user_profile_data(user_data)
                _render_avatar()
                self._build_nav_badge()
            except Exception as ex:
                messagebox.showerror("Error", f"Failed to save photo:\n{ex}", parent=popup)

        _render_avatar()

        ctk.CTkLabel(header, text=self.current_user,
                     font=("Trebuchet MS", 15, "bold"), text_color=TEXT_WHITE).pack()

        current_bio = user_data.get("bio", "Write your bio...")
        bio_var = ctk.StringVar(value=current_bio)

        bio_frame = ctk.CTkFrame(header, fg_color="transparent")
        bio_frame.pack(pady=(2, 12))

        bio_lbl = ctk.CTkLabel(bio_frame, textvariable=bio_var,
                               font=("Trebuchet MS", 11, "italic"),
                               text_color="#888888", cursor="hand2")
        bio_lbl.pack(side="left")

        edit_ico = ctk.CTkLabel(bio_frame, text="", font=("Arial", 11),
                                text_color="#555555", cursor="hand2")
        edit_ico.pack(side="left")

        def _start_edit_bio(e=None):
            bio_lbl.pack_forget()
            edit_ico.pack_forget()
            entry = ctk.CTkEntry(bio_frame, textvariable=bio_var,
                                 width=180, height=28,
                                 fg_color="#1A1A1A", border_color="#555",
                                 font=("Trebuchet MS", 11),
                                 placeholder_text="Write your bio...")
            entry.pack(side="left")
            entry.focus()

            def _save_bio(e=None):
                new_bio = bio_var.get().strip() or "Write your bio..."
                bio_var.set(new_bio)
            ...
            entry.bind("<Return>",   _save_bio)
            entry.bind("<FocusOut>", _save_bio)

        bio_lbl.bind("<Button-1>",  _start_edit_bio)
        edit_ico.bind("<Button-1>", _start_edit_bio)

        # ── Divider ───────────────────────────────────────────────────────────
        ctk.CTkFrame(popup, fg_color="#333", height=1, corner_radius=0).pack(fill="x")

        # ── Stats ─────────────────────────────────────────────────────────────
        stats_row = ctk.CTkFrame(popup, fg_color="#252525", corner_radius=0)
        stats_row.pack(fill="x")

        cols = ctk.CTkFrame(stats_row, fg_color="transparent")
        cols.pack(pady=10, padx=10)

        all_movies = len(self.watchlist_data)
        ...
        for num, lbl_text, color in [
            (all_movies, "All Movies", "#233D6D"),
            (watched,    "Watched",    "#3A4B38"),
            (watching,   "Watching",   "#6D3F23"),
            (planning,   "Planning",   "#6D2323"),
        ]:
            col = ctk.CTkFrame(cols, fg_color="transparent", width=60)
            col.pack(side="left", padx=4)
            col.pack_propagate(False)
            ctk.CTkLabel(col, text=str(num),
                         font=("Trebuchet MS", 20, "bold"), text_color=TEXT_WHITE).pack()
            badge = ctk.CTkFrame(col, fg_color=color, corner_radius=6)
            badge.pack()
            ctk.CTkLabel(badge, text=lbl_text,
                         font=("Trebuchet MS", 9, "bold"), text_color=TEXT_WHITE).pack(padx=6, pady=3)
        ...
        ctk.CTkButton(
            btn_frame, text="📓  My Diary", height=38,
            fg_color="#2A2A2A", hover_color="#333333",
            border_width=1, border_color="#444",
            font=("Trebuchet MS", 12, "bold"), text_color=TEXT_WHITE,
            corner_radius=10,
            command=lambda: [popup.destroy(), self._set_filter("diary")]
        ).pack(fill="x", pady=(0, 8))

        if hasattr(self.app, "show_page"):
            ctk.CTkButton(
                btn_frame, text="👤  Full Profile", height=38,
                fg_color="transparent", hover_color="#2A2A2A",
                border_width=1, border_color="#444",
                font=("Trebuchet MS", 12, "bold"), text_color=TEXT_GRAY,
                corner_radius=10,
                command=lambda: [popup.destroy(), self.app.show_page("profile")]
            ).pack(fill="x")
    ...
    def _render_empty(self):
        wrap = ctk.CTkFrame(self.movie_area, fg_color=BG_CARD, corner_radius=20)
        wrap.pack(fill="x", pady=20)
        ctk.CTkLabel(wrap, text="🎬", font=("Arial", 40)).pack(pady=(30, 5))
        ctk.CTkLabel(wrap, text="Nothing here yet.", font=("Trebuchet MS", 15, "bold"), text_color=TEXT_GRAY).pack()
        ctk.CTkLabel(wrap, text="Add a movie using the form above.", font=("Trebuchet MS", 12), text_color="#555").pack(pady=(4, 30))
    ...
    def _add_movie(self):
        ...
        if not t:
            messagebox.showwarning("Warning", "Movie title cannot be empty!")
            return
        if any(m.get("title", "").lower() == t.lower() for m in self.watchlist_data):
            messagebox.showwarning("Duplicate", "This movie is already in your watchlist!")
            return
        poster_path = ""
    ...
    def _delete_movie(self, movie):
        if messagebox.askyesno("Delete", f'Remove "{movie["title"]}" from your watchlist?'):
