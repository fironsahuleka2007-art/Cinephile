...
class WatchlistPage(ctk.CTkFrame):
    ...
    def _build_nav(self):
        ...
        self.nav_title_lbl = ctk.CTkLabel(
            nav, text=f"Cinema Logbook: {self.current_user}",
            font=("Helvetica", 18, "bold"), text_color=TEXT_WHITE
        )
        self.nav_title_lbl.pack(side="left", padx=10)

        # Tab filter pill — mirip nav dashboard
        pill = ctk.CTkFrame(nav, fg_color=BG_TAB, corner_radius=25, height=42)
        pill.place(relx=0.99, rely=0.5, anchor="e", x=-20)
        ...
        for lbl, st in tabs:
            btn = ctk.CTkButton(
                pill, text=lbl, width=100, height=34,
                fg_color=ACCENT if st == self.filter else "transparent",
                corner_radius=20,
                font=("Trebuchet MS", 12, "bold"),
                text_color=TEXT_WHITE,
                hover_color="#3A3A3A",
                command=lambda s=st: self._set_filter(s)
            )
            btn.pack(side="left", padx=4, pady=4)
            self._tab_btns[st] = btn

    def _update_tab_highlight(self):
        for st, btn in self._tab_btns.items():
            btn.configure(fg_color=ACCENT if st == self.filter else "transparent")
    ...
    def _build_form(self):
        ...
        ctk.CTkLabel(
            ...
            font=("Helvetica", 15, "bold"), text_color=TEXT_WHITE
        ...
        ctk.CTkOptionMenu(
            ...
            width=150, height=38,
            fg_color="#1E1E1E", button_color=ORANGE,
        ...
        ctk.CTkButton(
            row, text="+ Add Movie",
            width=120, height=38,
            fg_color=GREEN_BTN, hover_color="#2d5a27",
    ...
    def _refresh_stats(self):
        ...
        stats = [
            ("Total",    str(total),    "#1D2562"),
            ("Planning", str(planning), "#8F2A2A"),
            ("Watching", str(watching), "#23461f"),
            ("Watched",  str(watched),  "#EC7D28"),
        ]
    ...
    def _render_empty(self):
        ...
        ctk.CTkLabel(
            wrap, text="Your watchlist is empty.",
            font=("Trebuchet MS", 15, "bold"), text_color=TEXT_GRAY
        ).pack()
        ctk.CTkLabel(
            wrap, text="Add a movie above to get started.",
            font=("Trebuchet MS", 12), text_color="#555"
        ).pack(pady=(4, 30))
    ...
    def _render_standard_card(self, movie):
        ...
        status_color = {"Plan to Watch": "#8F2A2A", "Watching": GREEN_BTN, "Watched": "#EC7D28"}.get(status, TEXT_GRAY)
        meta = ctk.CTkFrame(info, fg_color="transparent")
        meta.pack(anchor="w", pady=(4, 0))
        ctk.CTkLabel(meta, text=year, font=("Trebuchet MS", 12), text_color=ORANGE).pack(side="left")
    ...
    def _render_diary_timeline(self, data):
        ...
        ctk.CTkLabel(
            hdr, text="My Movie Diary",
            font=("Georgia", 26, "bold"), text_color=TEXT_WHITE
        ).pack(side="left")
        ctk.CTkLabel(
            hdr, text=f"{len(data)} entries",
        ...
        for movie in data:
            ...
            ctk.CTkLabel(
                mid, text=movie.get("title", "Unknown"),
                font=("Helvetica", 17, "bold"),
                text_color=TEXT_WHITE, anchor="w"
            ).pack(fill="x")

            info_row = ctk.CTkFrame(mid, fg_color="transparent")
            info_row.pack(anchor="w", pady=(4, 8))
            ctk.CTkLabel(
                info_row,
                text=f"🗓️ {movie.get('watch_date', '–')}",
                font=("Trebuchet MS", 12, "bold"),
                text_color=ORANGE
            ).pack(side="left")
            ctk.CTkLabel(
            ...
            ctk.CTkLabel(
                info_row,
                text=f"📺 {movie.get('platform', 'Other')}",
                font=("Trebuchet MS", 12, "bold"),
                text_color="#2d5a27"
            ).pack(side="left")
    ...
    def _open_diary_popup(self, movie):
        ...
        ctk.CTkLabel(
            cont, text="📔 Movie Logbook",
            font=("Georgia", 22, "bold"), text_color=ORANGE
        ).pack(pady=(0, 6))
        ctk.CTkLabel(
            cont, text=movie.get("title", ""),
            font=("Trebuchet MS", 14), text_color=TEXT_GRAY
        ).pack(pady=(0, 20))

        ctk.CTkLabel(cont, text="Watch Date (YYYY-MM-DD)",
                     font=("Trebuchet MS", 12, "bold"), text_color=TEXT_WHITE).pack(anchor="w")
        e_date = ctk.CTkEntry(cont, height=38, fg_color="#1E1E1E", border_color="#444",
                              font=("Trebuchet MS", 13))
        e_date.pack(fill="x", pady=(5, 14))
        ...
        ctk.CTkLabel(cont, text="Personal Notes / Thoughts:",
                     font=("Trebuchet MS", 12, "bold"), text_color=TEXT_WHITE).pack(anchor="w")
        t_notes = ctk.CTkTextbox(
            cont, height=140, fg_color="#1E1E1E",
            border_width=1, border_color="#444",
            font=("Trebuchet MS", 12)
        )
        ...
        ctk.CTkButton(
            cont, text="Save to Diary",
            fg_color=GREEN_BTN, hover_color="#2f855a",
